    ...
    async function searchForReminders(e) {
        ...
        try {
            if (e.target.value === '') {
                ...
                const response = await fetch(`http://localhost:5000/dashboard/search/?title=${titleSearched}`, {
                    method: "GET",
                    headers: {"token": localStorage.token}
                });
                const parseResp = await response.json();
        ...
        <Fragment>
            <div className="search-container">
                ...
                <div className="reminders-container">
                        ...
                        <tbody>
                            {filteredReminders.map((currReminder) => (
                                <tr key={currReminder.reminder_id}>
                                    ...
                                    <td onClick={isAuth} >
                                        <EditReminder currReminder={currReminder} redirectTo="/dashboard/Search" />
                                    </td>
                                    <td onClick={isAuth} >
                                        <InlineConfirmButton
                    ...
                    {filteredReminders.length === 0 && <p>No results found.</p>}
                </div>
